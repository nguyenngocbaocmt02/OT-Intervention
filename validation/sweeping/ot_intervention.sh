...
for MODEL in llama_7B; do
    for alpha in 0.1; do
        for kappa in 20; do
            echo "model: $MODEL alpha: $alpha kappa: $kappa"
            CUDA_VISIBLE_DEVICES=3 python ot_edit_layer.py --exp test1 --model_name $MODEL --use_mode test --bl $BL --kappa $kappa --alpha $alpha --device 0 --num_fold 2 --judge_name $JUDGE --info_name $INFO --eval_dataset $EVAL_DATASET --train_dataset $TRAIN_DATASET
            echo
            echo
        done
    done
