...
for MODEL in llama3_8B; do
    for alpha in 0; do
        for K in 0; do
            ...
            CUDA_VISIBLE_DEVICES=1 python validate_2fold.py --prompting 50 --instruction_prompt default --model_name $MODEL --use_mode test --num_heads $K --alpha $alpha --device 0 --num_fold 2 --use_center_of_mass --judge_name $JUDGE --info_name $INFO --eval_dataset $EVAL_DATASET --train_dataset $TRAIN_DATASET
            echo
            echo
        done
    done
...
for MODEL in llama3_8B; do
    for alpha in 15; do
        for K in 48; do
            echo "model: $MODEL alpha: $alpha K: $K"
            CUDA_VISIBLE_DEVICES=1 python validate_2fold.py --prompting 50 --instruction_prompt default --model_name $MODEL --use_mode test --num_heads $K --alpha_ot $alpha --device 0 --num_fold 2 --use_center_of_mass --judge_name $JUDGE --info_name $INFO --eval_dataset $EVAL_DATASET --train_dataset $TRAIN_DATASET
            echo
            echo
        done
    done
